...
_MNIST_DIRECTORY = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'mnist.pkl.gz')
...
def load_data():
    ...
    below.
    """
    f = gzip.open(_MNIST_DIRECTORY, 'rb')
...
def load_data_wrapper():
    ...
    validation_inputs = np.array([np.reshape(x, (784, 1)) for x in va_d[0]])
    validation_results = np.array([vectorized_result(y) for y in va_d[1]])
    test_inputs = np.array([np.reshape(x, (784, 1)) for x in te_d[0]])
